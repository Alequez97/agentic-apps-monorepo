...
test("task socket bridge emits only to the owning user room", async () => {
  // Arrange
  ...
  // Assert
  assert.deepEqual(
    emitted.map(({ room, eventName }) => ({ room, eventName })),
    [
      { room: "user:user-a", eventName: SOCKET_EVENTS.TASK_QUEUED },
      { room: "user:user-a", eventName: SOCKET_EVENTS.TASK_PROGRESS },
      {
        room: "user:user-a",
        eventName: SOCKET_EVENTS.LOG_MARKET_RESEARCH_INITIAL,
      },
      {
        room: "user:user-b",
        eventName: SOCKET_EVENTS.MARKET_RESEARCH_REPORT_READY,
      },
    ],
  );

  assert.equal(
    emitted.some(({ room }) => room === "user:user-c"),
    false,
  );
});
